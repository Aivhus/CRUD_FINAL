...
function displayProtectedContent($db){
  ...
  <li class="nav-item">
    ...
    <li class="nav-item">
    <a class="nav-link" href="?logout=1" ><button type="button" class="btn btn-danger"><span class="cui-contrast"></span><h3>LOGOUT</h3></button></a>
  </li>
</ul>';
...
// UPLOAD
        if ($action == "update" ){
            formContact($nom,$prenom,$email,$message);
            $nom = filter_input(INPUT_POST,"nom",FILTER_SANITIZE_STRING);
            $prenom = filter_input(INPUT_POST,"prenom",FILTER_SANITIZE_STRING);
            $email = filter_input(INPUT_POST,"email",FILTER_SANITIZE_STRING);
            $message = filter_input(INPUT_POST,"message",FILTER_SANITIZE_STRING);
...
// UPLOAD
        if ($action== "update" ){
            formTableImage2($db);
            //formImage($nom_dossier,$nom_fichier,$extension,$position);
            $nom_dossier = filter_input(INPUT_POST,"nom_dossier",FILTER_SANITIZE_STRING);
            $nom_fichier = filter_input(INPUT_POST,"nom_fichier",FILTER_SANITIZE_STRING);
            $extension = filter_input(INPUT_POST,"extension",FILTER_SANITIZE_STRING);
            $position = filter_input(INPUT_POST,"position",FILTER_SANITIZE_STRING);
...
//UPDATE
        if ($action== "update" ){
            formTableLiens2($lines);
            //formLien($lien,$position);
            $lien = filter_input(INPUT_POST, "lien", FILTER_SANITIZE_STRING);
            $position = filter_input(INPUT_POST, "position", FILTER_SANITIZE_STRING);
            if (!empty($lien && $position)) {
